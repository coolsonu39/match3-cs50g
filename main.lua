...
function love.mousepressed(x, y, button, istouch)
    x, y = push:toGame(x, y)
    if gStateMachine.currentState == 'start' then
        -- VIRTUAL_WIDTH / 2 - 76, VIRTUAL_HEIGHT / 2 + y - 11, 150, 58
        if x >= VIRTUAL_WIDTH / 2 - 76 and y >= VIRTUAL_HEIGHT / 2 + 1 and x <= VIRTUAL_WIDTH / 2 + 74 and y <= VIRTUAL_HEIGHT / 2 + 40 then
            love.keypressed('start_game_button')
        elseif x >= VIRTUAL_WIDTH / 2 - 76 and y >= VIRTUAL_HEIGHT / 2 + 40 and x <= VIRTUAL_WIDTH / 2 + 74 and y <= VIRTUAL_HEIGHT / 2 + 59 then
            love.event.quit()
        end
    else
        love.keypressed('mouse_click')
    end
 end

...
function StartState:init()
    ...
    for i = 1, 64 do
        table.insert(positions, gFrames['tiles'][math.random(gColors)][math.random(6)])
    end
...
function StartState:update(dt)
    ...
    if not self.pauseInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or love.keyboard.wasPressed('start_game_button') then
            if self.currentMenuItem == 1 or love.keyboard.wasPressed('start_game_button') then

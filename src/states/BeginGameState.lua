...
function BeginGameState:init()
    ...
    -- self.board = Board(VIRTUAL_WIDTH - 272, 16, 1) --spec2 (extra 1 level var)
...
function BeginGameState:enter(def)
    ...
    self.isReset = def.resetBoard
    self.timer = def.timer

    self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level) --spec2
    --
...
function BeginGameState:render()
    ...
    if self.isReset then
        love.graphics.printf('No matches, board reset', 0, self.levelLabelY, VIRTUAL_WIDTH, 'center')
    else
